...
import { PencilIcon, TrashIcon, NoSymbolIcon, PlusIcon } from "@heroicons/react/24/outline";
...
interface Worker {
    ...
    phone: string;
    address: string;
    availability: Availability; // Use the Availability type
    status: string;
    password: string;
}
...
const generateTimeOptions = () => {
    const times = ["OFF"]; // Add "OFF" as the first option
...
const groupAvailability = (availability: Availability) => {
    ...
    for (let i = 1; i < days.length; i++) { // Change to i < days.length
        const day = days[i];
        const nextAvailability = availability[day as keyof Availability]; // No need for undefined check
        ...
        if (currentAvailability.startTime === "OFF") {
            grouped.push(`${startDay === lastDay ? `${startDay}` : `${startDay} - ${lastDay}`}: OFF`);
        } else {
            grouped.push(
...
export default function Workers() {
        ...
        const fetchWorkers = async () => {
            try {
                ...
                const workersData = querySnapshot.docs.map((doc) => ({
                    ...
                    availability: doc.data().availability as Availability, // Ensure it matches the type
    ...
    const handleEdit = (worker: Worker) => {
        ...
        setNewWorker({
            ...
            availability: worker.availability, // This now matches the type
    ...
    const handleSuspend = async (id: string) => {
        if (window.confirm("Are you sure you want to toggle the worker's status?")) {
            try {
                            ...
                            {Object.keys(newWorker.availability).map((day) => (
                                <div key={day} className="flex flex-col space-y-2">
                                    ...
                                    <div className="flex space-x-2">
                                        <select
                                            value={newWorker.availability[day as keyof Availability].startTime}
                                            onChange={(e) => {
                                                ...
                                                setNewWorker((prev) => ({
                                                    ...
                                                    availability: {
                                                        ...
                                                        [day]: {
                                                            startTime: value,
                                                            endTime: value === "OFF" ? "OFF" : prev.availability[day as keyof Availability].endTime,
                                                        },
                                                    },
                                                }));
                                            }}
                                        ...
                                        <select
                                            value={newWorker.availability[day as keyof Availability].endTime}
                                            onChange={(e) => {
                                                const value = e.target.value;
                                                setNewWorker((prev) => ({
                                                    ...prev,
                                                    availability: {
                                                        ...prev.availability,
                                                        [day]: {
                                                            startTime: value === "OFF" ? "OFF" : prev.availability[day as keyof Availability].startTime,
                                                            endTime: value,
                                                        },
                                                    },
                                                }));
                                            ...
                                            className="p-2 border border-gray-300 rounded-lg"
                                            disabled={newWorker.availability[day as keyof Availability].startTime === "OFF"}
                                        >
                                            {timeOptions.map((time) => (
                                                <option key={time} value={time}>
                                                    {time}
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {workers.map((worker) => {
                            const groupedAvailability = groupAvailability(worker.availability);
                            return (
                                <tr
                                    key={worker.id}
